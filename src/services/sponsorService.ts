...
class SponsorService {
  ...
  //GET all sponsors full - VIEW: Admin & concerning team
  ...
  {
    const sponsors = await this.sponsorRepository.createQueryBuilder('sponsor')
  ...
  async getAllSponsorsLimitedForTeam(teamId:number): Promise<Partial<Sponsor>[]>
  {
    const sponsors = await this.sponsorRepository.createQueryBuilder('sponsor')
    .leftJoinAndSelect('sponsor.teams','team') 
    .select([
  ...
  //GET sponsor full by Id - VIEW: Admin & concerning team
  ...
  async updateSponsor(id: number, updatedSponsor: Sponsor): Promise<SponsorEntity> {
    const sponsorToBeUpdated = await this.sponsorRepository.findOneBy({ sponsorId: id });  
