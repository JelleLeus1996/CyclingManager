...
class RiderService {
  ...
  //GET all riders full - VIEW: Admin & concerning team
  ...
  {
    const riders = await this.riderRepository.createQueryBuilder('rider') 
    .select([
      'rider.first_name',
      'rider.last_name',
      'rider.nationality',
  ...
  //GET rider full (riderId) - VIEW: Admin & concerning team
  async getRiderById(id: number): Promise<Rider | undefined> {
    ...
    async getLimitedRiderById(riderId: number): Promise<Rider | undefined> {
      const rider = await this.riderRepository.createQueryBuilder('rider') 
      .select([
        'rider.first_name',
        'rider.last_name',
        'rider.nationality',
    ...
    //GET riders full from teamId - VIEW: Admin & concerning team
    async getRidersFromTeam(teamId: number): Promise<Rider[]> {
      const riders  = await this.riderRepository.find({ where: { teamId: teamId }});
      return riders;
    }
  ...
  //GET rider full by name (first_name & last_name) - VIEW: Admin & concerning team
  async getRiderByName(first_name: string, last_name: string): Promise<Rider | undefined> {
    const rider = await this.riderRepository.findOneBy({ first_name: first_name, last_name: last_name });
    return rider as Rider;
  }

    //GET rider limited by name (first_name & last_name) - VIEW: ALL
    async getLimitedRiderByName(first_name: string, last_name: string): Promise<Rider | undefined> {
      const rider = await this.riderRepository.createQueryBuilder('rider') 
      .select([
        'rider.first_name',
        'rider.last_name',
  ...
  async updateRider(id: number, updatedRider: Rider): Promise<RiderEntity> {
    const riderToBeUpdated = await this.riderRepository.findOneBy({ riderId: id });
    if (!riderToBeUpdated) {
